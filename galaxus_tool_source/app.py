# app.py
# Galaxus Sell‑out Aggregator – robust, mit KW-Dropdown-Filter und Farb-/Kategoriehandling
...
def prepare_price_df(df: pd.DataFrame) -> pd.DataFrame:
    ...
    if col_color:
        out["Farbe"] = df[col_color].fillna("").astype(str).replace(["nan", "None"], "")
    else:
        out["Farbe"] = ""
...
def _find_any_date_column(df):
    # Fallback: suche die erste Spalte, die sich sinnvoll als Datum parsen lässt
...
def prepare_sell_df(df: pd.DataFrame) -> pd.DataFrame:
    ...
    # 1) Direkte Datumsspalte
    if col_date:
        out["Datum"] = parse_date_series(df[col_date])
    # 2) Start-/End-Datum -> nutze Start für KW-Bestimmung
    elif col_start:
        out["Datum"] = parse_date_series(df[col_start])
    # 3) KW/Jahr -> konstruiere Montags-Datum
...
st.caption(
    "Lädt Preislisten & Sell-out-Daten, matcht robust und berechnet Einkaufs-/Verkaufs-/Lagerwerte. "
    "Anzeige zeigt gerundete Summen mit Tausendertrennzeichen."
)
...
if sell_file and price_file:
    try:
        ...
        # Periodenfilter (KW/Jahr) – nur wenn wir Datums-/KW-Infos erzeugen konnten
        ...
        if "Datum" in sell_df.columns and not sell_df["Datum"].isna().all():
            ...
            if periods:
                ...
                selected_period = st.selectbox(
                    ...
                    index=0,
                    help="Ermittelt aus der Datumsspalte bzw. aus Start/KW/Jahr."
                )
                if selected_period != "Alle":
                    filtered_sell_df = sell_df[sell_df["Period"] == selected_period].copy()
        if not period_enabled:
            st.info("Hinweis: Keine Datum-/KW-Information im Sell-out erkannt – Filter ausgeblendet. "
                    "Akzeptierte Spalten: "
        ...
        # Nur die Summen anzeigen (Aggregierte Werte)
