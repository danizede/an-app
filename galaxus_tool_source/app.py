...
# Robustes Matching: ArtNr → EAN → Name → Kurzname → Hints (deine Regeln)
# EU-Datumsfilter, Detailtabelle optional, Farbvarianten nur bei Dubletten
# Overflow-sicher (float64 + Sanity-Clean), Length-mismatch/Scope-Fixes
...
# =========================
# Robust einlesen (gegen Length-mismatch)
# =========================
def read_excel_flat(upload) -> pd.DataFrame:
    ...
    headers = raw.iloc[header_idx].fillna("").astype(str).map(lambda h: re.sub(r"\s+"," ",h).strip()).tolist()
    n = raw.shape[1]
    if len(headers) < n: headers += [f"col_{i}" for i in range(len(headers), n)]
    else: headers = headers[:n]
    df = raw.iloc[header_idx+1:].reset_index(drop=True)
    df.columns = headers
    # Duplikate eindeutiger machen
    seen, nc = {}, []
    for c in df.columns:
        k = (c, seen.get(c,0)); nc.append(c if k[1]==0 else f"{c}.{k[1]}")
...
def find_column(df: pd.DataFrame, candidates, purpose: str, required=True) -> str|None:
    ...
    if required: raise KeyError(f'Spalte für «{purpose}» fehlt – gesucht unter {candidates}.\nVerfügbare Spalten: {cols}')
    return None
...
def sanitize_numbers(qty: pd.Series, price: pd.Series) -> tuple[pd.Series,pd.Series]:
    ...
    return q.where((q>=0)&(q<=MAX_QTY)), p.where((p>=0)&(p<=MAX_PRICE))
    ...
    for rgx in _COLOR_REGEXES:
        ...
        if m:
            ...
            if not _looks_like_not_a_color(cand):
                return _canon_color_from_text(cand) or cand
    c=_canon_color_from_text(name)
    return c if c and not _looks_like_not_a_color(c) else ""
...
def guess_color_from_row(row: pd.Series, all_columns: list[str]) -> str:
    for col in all_columns:
        if re.search(r"(farb|color|colour|varian)", col, re.IGNORECASE):
            ...
            if canon and not _looks_like_not_a_color(canon): return canon
    for col in all_columns:
        if col.lower() in {"ean","gtin","barcode","artikelnummer","artikelnr","artnr","produkt id"}: continue
        val=str(row.get(col,"") or "").strip()
        if not val or re.fullmatch(r"[0-9 .,'’-]+", val): continue
        c=extract_color_from_name(val) or _canon_color_from_text(val)
        if c and not _looks_like_not_a_color(c): return c
    return ""
...
def prepare_price_df(df: pd.DataFrame) -> pd.DataFrame:
    ...
    if col_color:
        ...
        if out["Farbe"].isna().any() or (out["Farbe"].astype(str).str.strip()=="").any():
            ...
            out["Farbe"] = out.apply(lambda r: r["Farbe"] if str(r.get("Farbe","")).strip() else
                                     guess_color_from_row(df.loc[r.name], all_cols), axis=1)
    out["Farbe"] = out["Farbe"].fillna("").astype(str)

    # Lager & Preise
    out["Lagermenge"] = parse_number_series(df[col_stock]).fillna(0).astype("Int64") if col_stock else pd.Series([0]*len(df), dtype="Int64")
    ...
    if not col_buy and not col_sell and col_any:
        price_any = parse_number_series(df[col_any]); out["Einkaufspreis"]=price_any; out["Verkaufspreis"]=price_any
...
def _apply_hints_to_row(name_raw: str) -> dict:
    """ Deine Regeln: liefert evtl. Hint-Kurzkey, Hint-Farbe, Hint-ArtNr (prefix/exakt) """
    s = (name_raw or "").lower()
    hints = {"hint_kurz": "", "hint_color": "", "hint_artnr_exact":"", "hint_artnr_prefix":""}

    if "mia" in s and "gold" in s:
        hints["hint_color"] = "Schwarz"             # Mia gold -> Schwarz
    if "otto" in s:
        hints["hint_artnr_prefix"] = "o013"         # Otto -> O-013
    if "oskar" in s and "little" in s:
        hints["hint_color"] = "Schwarz"             # Oskar little -> Schwarz
    if any(x in s for x in ["peter","robert","roger"]):
        hints["hint_kurz"] = "roger"                # auf Roger mappen
        hints["hint_color"] = hints["hint_color"] or "Schwarz"
    if "simon" in s:
        hints["hint_artnr_exact"] = "s054"          # Simon -> S-054
    if "tim" in s and "schwarz" in s:
        hints["hint_color"] = "Weiss"               # Tim schwarz -> Weiss
    if "finn" in s:
        hints["hint_kurz"] = "finn"                 # Finn + Finn mobile
    if "theo" in s:
        hints["hint_kurz"] = "theo"                 # Theo Familie
    if "albert" in s:
        hints["hint_kurz"] = "albert"
    return hints
...
def prepare_sell_df(df: pd.DataFrame) -> pd.DataFrame:
    ...
    col_start= find_column(df, DATE_START_CANDS, "Startdatum (Spalte I)", required=False) or _fallback_col_by_index(df,8) if df.shape[1]>=9 else None
    col_end  = find_column(df, DATE_END_CANDS,   "Enddatum (Spalte J)",   required=False) or _fallback_col_by_index(df,9) if df.shape[1]>=10 else None
    ...
    if "StartDatum" in out and "EndDatum" in out:
        mask = out["EndDatum"].isna(); out.loc[mask,"EndDatum"] = out.loc[mask,"StartDatum"]
    return out

# =========================
# Merge & Berechnung
# =========================
def _f64(s: pd.Series) -> pd.Series: return pd.to_numeric(s, errors="coerce").astype("float64")
...
def _match_with_hints(merged: pd.DataFrame, price_df: pd.DataFrame):
    ...
    for i in idxs:
        ...
        hint_prefix= str(merged.at[i,"Hint_ArtNr_Pre"] or "")
        hint_kurz  = (merged.at[i,"Hint_Kurz"] or "").strip()
        hint_color = (merged.at[i,"Hint_Color"] or "").strip()

        # 1) exakte ArtikelNr-Hint
        if hint_artnr:
            ak = normalize_key(hint_artnr)
            hit = price_df.loc[price_df["ArtikelNr_key"]==ak]
            if not hit.empty:
                _assign_from_price_row(merged, i, hit.iloc[0]); continue
        # 2) ArtikelNr-Prefix-Hint
        if hint_prefix:
            pref = hint_prefix.lower()
            hit = price_df.loc[price_df["ArtikelNr_key"].str.startswith(pref, na=False)]
            if not hit.empty:
                # Farbe bevorzugen
                if hint_color:
                    hitc = hit.loc[hit["Farbe"].str.lower()==hint_color.lower()]
                    if not hitc.empty: hit = hitc
                _assign_from_price_row(merged, i, hit.iloc[0]); continue
        # 3) Kurzname-Hint
        if hint_kurz:
            group = price_by_kurz.get_group(hint_kurz) if hint_kurz in price_by_kurz.groups else pd.DataFrame()
            if not group.empty:
                if hint_color:
                    g2 = group.loc[group["Farbe"].str.lower()==hint_color.lower()]
                    if not g2.empty: group = g2
                _assign_from_price_row(merged, i, group.iloc[0]); continue
...
def enrich_and_merge(sell_df: pd.DataFrame, price_df: pd.DataFrame) -> tuple[pd.DataFrame,pd.DataFrame]:
    ...
    need = merged["Verkaufspreis"].isna()
    if need.any():
        mapping = price_df.drop_duplicates("Bezeichnung_key").set_index("Bezeichnung_key")
        for i,k in zip(merged.index[need], merged.loc[need,"Bezeichnung_key"]):
            if k in mapping.index: _assign_from_price_row(merged, i, mapping.loc[k])

    # Fallback Kurzname
    need = merged["Verkaufspreis"].isna()
    if need.any():
        mapping = price_df.drop_duplicates("Kurz_key").set_index("Kurz_key")
        for i,k in zip(merged.index[need], merged.loc[need,"Kurz_key"]):
            if k and k in mapping.index: _assign_from_price_row(merged, i, mapping.loc[k])

    # NEU: Hints (deine Regeln)
    _match_with_hints(merged, price_df)

    # Strings säubern + Anzeige
...
if sell_file and price_file:
    try:
        ...
        raw_sell, raw_price = read_excel_flat(sell_file), read_excel_flat(price_file)

        with st.spinner("📖 Lese & prüfe Spalten…"):
            sell_df, price_df = prepare_sell_df(raw_sell), prepare_price_df(raw_price)
        ...
        if show_detail:
            ...
            d_rounded, d_styler = style_numeric(detail); st.dataframe(d_styler, use_container_width=True)

        st.subheader("Summen pro Artikel (Varianten: Farbe bei Dubletten)")
        t_rounded, t_styler = style_numeric(totals); st.dataframe(t_styler, use_container_width=True)

        d1,d2 = st.columns(2)
        with d1:
            st.download_button("⬇️ Detail (CSV)",
