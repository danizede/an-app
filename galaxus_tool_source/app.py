# app.py
# Galaxus Sell‑out Aggregator – robust, stabil, mit Datumsfilter (Spalte I/J, US->EU)
...
def style_numeric(df: pd.DataFrame, num_cols=NUM_COLS_DEFAULT, sep=THOUSANDS_SEP):
    out = df.copy()
    # Bugfix: Generator korrekt (kein freies 'c')
...
def parse_date_series_us(s: pd.Series) -> pd.Series:
    """US-Format (MM/DD/YYYY) sicher parsen; falsche Werte -> NaT."""
    # Wenn Excel bereits datetime liefert, nicht kaputt-parsen
    if np.issubdtype(s.dtype, np.datetime64):
        return s
    return pd.to_datetime(s, errors="coerce", dayfirst=False, infer_datetime_format=True)

def format_eu_date(d: pd.Series | pd.DatetimeIndex) -> pd.Series:
    return pd.to_datetime(d, errors="coerce").dt.strftime("%d.%m.%Y")
...
# =========================
# Parsing – Sell‑out‑Report (mit Spalte I/J für Start/Ende)
# =========================
...
def _fallback_col_by_index(df: pd.DataFrame, index_zero_based: int) -> str | None:
    """Gibt den Spaltennamen an einer festen Position zurück (0‑basiert), falls vorhanden."""
    try:
...
def prepare_sell_df(df: pd.DataFrame) -> pd.DataFrame:
    ...
    # Zuerst versuchen: benannte Start/Ende-Spalten
    ...
    # Fallback strikt nach Position: I (Index 8) & J (Index 9)
    if not col_start and len(df.columns) >= 9:
        col_start = _fallback_col_by_index(df, 8)  # Spalte I
    if not col_end and len(df.columns) >= 10:
        col_end   = _fallback_col_by_index(df, 9)  # Spalte J
    ...
    # US -> EU Datumsverarbeitung (Start/Ende)
    if col_start:
...
if sell_file and price_file:
    try:
        ...
        # --- Zeitraumauswahl (über Start/Ende aus Spalte I/J) ---
        ...
        if {"StartDatum", "EndDatum"}.issubset(sell_df.columns) and not sell_df["StartDatum"].isna().all():
            ...
            start_date, end_date = st.date_input(
                "Zeitraum auswählen (DD.MM.YYYY)",
                value=(min_date, max_date),
                min_value=min_date,
                max_value=max_date,
                help="Filtert auf Zeilen, deren Zeitraum (Start/Ende) sich mit der Auswahl überschneidet."
            )
            # Überschneidung: !(Ende < Start) und !(Start > Ende)
            mask = ~(
