...
_COLOR_MAP = {
    ...
    "schwarz":"Schwarz","black":"Black","grau":"Grau","gray":"Grau","anthrazit":"Anthrazit","charcoal":"Anthrazit",
    "graphite":"Graphit","silver":"Silber",
...
def prepare_price_df(df: pd.DataFrame) -> pd.DataFrame:
    ...
    # Deduplizieren nach ArtikelNr_key – behalte Zeile mit Preis, sonst erste
...
def _apply_hints_to_row(name_raw: str) -> dict:
    ...
    # Tim schwarz -> Tim weiss
    if "tim" in s and "schwarz" in s:
        h["hint_kurz"]="tim"
        h["hint_color"]="weiss"
    # Finn, Finn mobile, Theo, Robert, Peter, Julia, Albert nach Namen
    for fam in ["finn","theo","robert","peter","julia","albert"]:
        if fam in s:
            h["hint_kurz"] = fam
    if "finn mobile" in s:
        h["hint_kurz"] = "finn"
    # Roger – nur Roger bleibt Roger (Schwarz)
    if "roger" in s:
        h["hint_kurz"] = "roger"
...
def _fallback_col_by_index(df: pd.DataFrame, idx0: int) -> str | None:
    """Backup-Spaltenname anhand fester Position (0-basiert) – für Datumsspalten I/J."""
    try:
...
if sell_file and price_file:
    try:
        ...
        if {"StartDatum","EndDatum"}.issubset(sell_df.columns) and not sell_df["StartDatum"].isna().all():
            ...
            overlaps=~((sell_df["EndDatum"].dt.date < start_date) |
